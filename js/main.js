...
function initialize() {
  ...
  localVideo.addEventListener('loadeddata', function checkDim(){
    if (this.videoWidth && this.videoHeight) {
      window.onresize();  
    } else {
      // this event doesnt really work so
      // keep checking until it does
      setTimeout(checkDim.bind(this), 100);
    }
  });
...
function bindEvents () {
  ...
  dataChannel.onmessage = function (e) {
    console.log("GOT DATA", e.data, e)
    writeMessage(e.data, "recv");
  };
}

function writeMessage (data, who) {
  var now = new Date();
  var format = now.getHours() + ":" + now.getMinutes();
  var msglog = document.getElementById("msglog");
  msglog.innerHTML += "<div class='"+who+"'><span>"+format+"</span>" + data + "</div>"
...
window.onresize = function(){
  ...
  var videoWidth = innerWidth < aspectRatio * (window.innerHeight - textHeight) ?
                   innerWidth : aspectRatio * (window.innerHeight  - textHeight);
